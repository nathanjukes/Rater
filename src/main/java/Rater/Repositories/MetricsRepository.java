...
public interface MetricsRepository extends JpaRepository<RequestMetric, UUID> {
    ...
    @Query(value = "SELECT m.api_id, a.name, a.http_method, COUNT(m.api_id) AS acceptedRequests FROM metrics m JOIN apis a ON m.api_id = a.id WHERE m.org_id = ?1 AND m.request_accepted = 'true' GROUP BY m.api_id, a.name ORDER BY acceptedRequests DESC LIMIT 5", nativeQuery = true)
    List<Object[]> getOrgMostAcceptedAPIs(UUID orgId);

    @Query(value = "SELECT m.api_id, a.name, a.http_method, COUNT(m.api_id) AS deniedRequests FROM metrics m JOIN apis a ON m.api_id = a.id WHERE m.org_id = ?1 AND m.request_accepted = 'false' GROUP BY m.api_id, a.name ORDER BY deniedRequests DESC LIMIT 5", nativeQuery = true)
